...
class UserController extends MyFct
{
    ...
    function listerUser()
    {
        ...
        foreach ($users as $value) {
            ...
            $roles = json_decode($user->getRoles(), true); // the second parameter ensures the result is an array
            if (is_array($roles)) {
                $role_title = implode(" - ", $roles);
            } else {
                $role_title = '';
            } // transformer le tableau $roles en texte avec un separateur " - "
    ...
    function generateFormUser($user, $disabled)
    {
        ...
        foreach ($myRoles as $role) {
            ...
            if (is_array($user_roles) && in_array($libelle, $user_roles)) {  // si $libelle fait parti de la tables $user_roles
                $selected = "selected";
                $checked = "checked";
            } else {
                $selected = "";
